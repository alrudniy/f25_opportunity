...
class CompanyAboutAccessTest(TestCase):
    ...
    def test_unauthenticated_user_access_company_about(self):
        # Attempt to access the company_about page without logging in
        response = self.client.get(reverse('company_about'))

        # Assert that the user is redirected (status code 302)
        self.assertEqual(response.status_code, 302)

        # Assert that the user is redirected to the login page (LOGIN_URL)
